...
class PerpMomentumAgent(PerpTradingAgent):
    ...
    def __init__(self, id, name, type, symbol='ASSET-USD', starting_cash=100000.0,
                 ...
                 trade_on_signal_change_only=False, crossover_deadband_bps=5.0,
                 **kwargs):
    ...
    def placeOrders(self, bid, ask):
        if bid is not None and ask is not None:
            ...
            mid = self._strategy_bootstrap_reference_price(self.symbol)
        ...
        avg_20 = self._ma(self.mid_list, n=min(20, len(self.mid_list)))
        avg_50 = self._ma(self.mid_list, n=min(50, len(self.mid_list)))
        self.avg_20_list.append(avg_20)
        self.avg_50_list.append(avg_50)

        spread_signal_bps = ((avg_20 - avg_50) / mid) * 10000.0 if mid > 0 else 0.0
